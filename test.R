simulation <- function(sample.size = 500, p = 500, alpha=0, rate=0){
  ...
  #                        Lam2=0, opts=list(init=0,  tol=10^-6, maxIter=1500), nfolds=5,
  #                        stratify=TRUE, Lam1_seq=10^seq(1,-4, -1), G=c(1,-1))
  #fit_logistic_mtl2 <- RMTL::MTL(list(x,x), list(y1=c(2*(y.cutoff[[1]]-0.5)),y2=c(2*(y.cutoff[[2]]-0.5))), type="Classification", Regularization="Graph",
  #                               Lam1=mtl2.cvfit$Lam1.min, Lam2=0, opts=list(init=0,  tol=10^-6, maxIter=1500), G=c(1,-1))
  ...
  score.class.logistic.mtl1 <- mean((x.test %*% fit_logistic_mtl1$W[,1] >= -fit_logistic_mtl1$C[1]) == y.test.label)
  ...
  score.rank.logistic.mtl1 <- cor(x.test %*% fit_logistic_mtl1$W[,1],y=y.test.nonoise, method='kendall')
